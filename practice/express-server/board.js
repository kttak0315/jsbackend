...
// req.bosy를 사용하려면 JSON 미들웨어를 사용해야 합니다.
// 사용하지 않으면 undefined로 전환
app.use(express.json()); // JSON 미들웨어 활성화

// POST 요청 시 컨택트 타입이 application/x-www-form-urlencoder인 경우 파싱
...
app.post("/posts", (req, res) => { // /posts로 요청이 오면 실행
    ...
    // 게시글을 작성할 때 필요한 데이터가 모두 있는지 확인게시글 리스트에 새로운 게시글 정보 추가
